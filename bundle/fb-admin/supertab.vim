...
" Modified By: Charlie Cheever <ccheever@facebook.com>
"
" Version: 0.32
" Description:
...
if !exists('complType') "Integration with other copmletion functions...
  ...
  fu! <SID>SuperTab()
    if (strpart(getline('.'),col('.')-2,1)=~'^\s\?$')
      return "\<Tab>"
    el
      " with omni-completion, we should just do that
      " instead of this other fancy crap --ccheever
      if g:omniCompletion == 1
        return "\<C-X>\<C-O>"
      el
        return g:complType
      en
